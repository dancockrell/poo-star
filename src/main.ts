...
async function rollReels(result:Result){
 ...
 const columns=Array.from({length:5},(_,col)=>{
  ...
  const lead=18+col*3;
  strip.innerHTML=Array.from({length:lead+3},(_,i)=>i<3?old[i*5+col].outerHTML:tile((i*7+col*3+state.round)%12,-1)).join('')+Array.from({length:3},(_,row)=>tile(result.grid[row*5+col],row*5+col)).join('');
 ...
 await Promise.all(columns.map(async({strip,distance,col})=>{
  const animation=strip.animate([{transform:'translateY(0)',filter:'blur(0)'},{transform:`translateY(${-distance*.08}px)`,filter:'blur(1.5px)',offset:.12},{transform:`translateY(${-distance*.92}px)`,filter:'blur(2px)',offset:.72},{transform:`translateY(${-distance-8}px)`,filter:'blur(0)',offset:.94},{transform:`translateY(${-distance}px)`,filter:'blur(0)'}],{duration:1300+col*210,easing:'cubic-bezier(.18,.55,.25,1)',fill:'forwards'});
