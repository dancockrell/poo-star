...
async function rollReels(result:Result){
 ...
 await Promise.all(columns.map(async({strip,distance,col})=>{
  ...
  ],{duration:1100+col*120,easing:'linear',fill:'forwards'});
...
async function play(){if(busy)return;let nextSpinDelay=300;busy=true;sound.beginRound();clearTimeout(burstTimer);el('poo-burst').replaceChildren();el('celebration').hidden=true;refresh();sound.play('roll');el('message').textContent='YOUR MOMENT IN THE SPOTLIGHT…';
 try{const result=spin(state,BETS[betIndex],browserRng);state=result.state;save();nextSpinDelay=result.awarded||result.payout>=result.bet*5?2200:result.payout?1400:300;
 ...
 if(auto){const epoch=autoEpoch;await delay(reduced?Math.min(nextSpinDelay,700):nextSpinDelay);if(auto&&epoch===autoEpoch&&!busy)void play();}}
...
// Try immediately where allowed; interaction retries respect explicit mute.
sound.resume();
for(const event of ["pointerdown","keydown"])document.addEventListener(event,e=>{if(!(e.target as HTMLElement)?.closest?.("#sound"))sound.resume();},{capture:true});
