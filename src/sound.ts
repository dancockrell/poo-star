...
export class Sound {
 enabled=false;musicLevel=.2;effectLevel=1;
 ...
 constructor(){try{const saved=JSON.parse(localStorage.getItem('poo-star-audio')||'null');if(saved){this.musicLevel=Math.max(0,Math.min(1,saved.music??.2));this.effectLevel=Math.max(0,Math.min(1,saved.effects??1));if(saved.mixVersion!==2){this.musicLevel=Math.min(.2,this.musicLevel);if(this.effectLevel>0)this.effectLevel=1;}}}catch{}}
 ...
 this.track=new Audio(musicUrl);this.track.loop=true;this.track.preload="auto";const trim=c.createGain();trim.gain.value=.45;c.createMediaElementSource(this.track).connect(trim);trim.connect(this.music);
 ...
 private poop(t:number,variant:number){const b=this.fx!;
  if(variant%3===0){this.squelch(92,t,.65,.19,b,true);this.hiss(t,.36,.14,360,b,130);}
  else if(variant%3===1){[0,.13,.31].forEach((offset,i)=>{this.note(420-i*60,t+offset,.2,.26,'sine',b,55);this.hiss(t+offset+.04,.2,.16,800,b,250);});}
  else {this.squelch(150,t,.32,.17,b,true);this.squelch(85,t+.27,.48,.2,b,true);}
 }
 ...
 if(kind==='roll'){this.duck(2.2,.35);this.poop(t,Math.floor(this.step/16));this.note(90,t,.16,.13,'triangle',b,45);this.hiss(t+.07,1.05,.13,420,b,2100);for(let i=0;i<8;i++)this.hiss(t+.1+i*.075,.025,.065,1500+i*140,b);}
 ...
 const big=kind!=='win';this.poop(t+.06,1);if(big)this.poop(t+.5,2);this.duck(big?3:1.3,big?.24:.42);
