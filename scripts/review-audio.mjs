...
await page.goto(process.env.DEMO_URL||'http://127.0.0.1:8791/',{waitUntil:'domcontentloaded'});await page.waitForTimeout(1400);if(await page.locator('#sound').getAttribute('aria-pressed')!=='true')throw Error('Sound should start enabled');
const measure=()=>page.evaluate(async()=>{const p=window.audioProbe[0],data=new Float32Array(p.meter.fftSize);let energy=0,peak=0;for(let i=0;i<12;i++){p.meter.getFloatTimeDomainData(data);energy+=data.reduce((sum,v)=>sum+v*v,0)/data.length;peak=Math.max(peak,...data.map(Math.abs));await new Promise(r=>setTimeout(r,50));}return {state:p.context.state,rms:Math.sqrt(energy/12),peak};});
