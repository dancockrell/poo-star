...
export class Sound {
 enabled=false;musicLevel=.35;effectLevel=.8;
 ...
 constructor(){try{const s=JSON.parse(localStorage.getItem('poo-star-audio')||'null');if(s){this.musicLevel=s.music===0?0:s.mixVersion===3?this.clamp(s.music):.35;this.effectLevel=s.effects===0?0:s.mixVersion===3?this.clamp(s.effects):.8;}}catch{}}
 ...
 levels(music:number,effects:number){this.musicLevel=this.clamp(music);this.effectLevel=this.clamp(effects);if(this.context){this.music!.gain.setTargetAtTime(this.musicLevel*this.duckDepth,this.context.currentTime,.05);this.fx!.gain.setTargetAtTime(this.effectLevel,this.context.currentTime,.05);}try{localStorage.setItem('poo-star-audio',JSON.stringify({mixVersion:3,music:this.musicLevel,effects:this.effectLevel}));}catch{}}
 ...
 if(kind==='roll'){this.sample('card-shuffle.ogg',0,.65);this.sample(fart,.08,.8);}
 if(kind==='stop')this.sample('chip-lay-1.ogg',0,.55+step*.04);
 if(kind==='scatter')this.sample('chips-collide-1.ogg',0,.8);
 if(kind==='lose')this.sample(fart,0,.95);
 if(kind==='wild'){this.duck();this.sample(fart);this.sample('splat.mp3',.4);}
