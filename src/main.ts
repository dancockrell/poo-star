...
function pooBurst(result:Result){
 ...
 const count=reduced?3:result.payout>=result.bet*5?16:7;
 layer.innerHTML=Array.from({length:count},(_,i)=>`<span class="poo-flight" style="--x:${8+(i*29)%84}%;--y:${18+(i*23)%55}%;--delay:${reduced?0:i*.065}s;--turn:${i%2?35:-40}deg"><span class="poo-piece">&#128169;</span><span class="poo-splat"></span></span>`).join('');
 burstTimer=setTimeout(()=>layer.replaceChildren(),reduced?900:3100);
}
function celebrate(result:Result){pooBurst(result);clearTimeout(celebrationTimer);if(!result.awarded&&!result.event&&result.payout<result.bet*5)return;
